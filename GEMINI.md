...
* **Sécurité Absolue des Clés d'API & Identifiants** : Les fichiers `gsc-credentials.json`, `*credentials*.json`, ainsi que les variables d'environnement locales (`BING_WEBMASTER_API_KEY`, `BRAVE_SEARCH_API_KEY`, `.env.local`, `.venv/`) doivent rester strictement exclus du versioning Git via `.gitignore`.
* **Décisions SEO & GEO Guidées par la Donnée Réelle** : Toute modification de `<title>`, meta description ou balisage Hn sur les pages pSEO (`/entretien/*`) doit s'appuyer sur les métriques réelles extraites de l'API Search Console (`npm run gsc`), de Bing Webmaster (`npm run bing`) ou de Brave Search (`npm run brave`), et non sur des conjectures.
* **Soumission d'Indexation Systématique & WDP** : À chaque ajout de nouvelle fiche d'entretien dans `src/data/maintenance/`, soumettre immédiatement les URLs générées à Bing (`npm run bing:submit`) pour indexation dans Bing et ChatGPT Search, sur Brave Search (`npm run brave:submit` vers `https://search.brave.com/submit-url`) pour la couverture Claude, et lancer l'accélérateur WDP (`npm run brave:wdp`).
* **Plafonnement & Pagination API Brave** : L'API REST Brave Web Search plafonne strictement les requêtes à `count <= 20` (erreur HTTP 422 si dépassé). Toute exploration du domaine doit borner `count` et paginer séquentiellement avec `offset`.
...
* **Outillage Standardisé** : Les analyses de visibilité s'exécutent exclusivement via l'environnement virtuel `.venv` et les scripts centralisés `scripts/gsc_analyzer.py`, `scripts/bing_analyzer.py` et `scripts/brave_analyzer.py`.
