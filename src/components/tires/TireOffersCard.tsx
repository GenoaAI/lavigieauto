...
export function TireOffersCard({
  ...
  return (
    <CollapsibleModuleCard
      ...
      title="Comparateur de Prix & Forfait Pose"
      subtitle={`Coût total transparent incluant l'achat des pneumatiques, le montage, la valve et l'équilibrage${vehicleName ? ` pour ${vehicleName}` : ""}`}
      badge={
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-xs font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded border border-slate-200 hidden sm:inline-block">
            {dimension}
          </span>
          {data?.offers?.[0]?.totalPrice && (
            <span className="text-xs font-extrabold text-emerald-700 bg-emerald-100/70 px-2 py-0.5 rounded-md hidden md:inline-block">
              Dès {data.offers[0].totalPrice.toFixed(0)} € TTC
            </span>
          )}
        </div>
      }
      actions={
        <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              fetchOffers(dimension, quantity);
            }}
            disabled={loading}
      ...
      className={`border-slate-200/90 ${className}`}
