...
export function ReservationKitModal({
  ...
  const totalBudget = kit.interventionsToRequest.reduce((acc, i) => acc + (i.estimatedBudgetEur || 0), 0);

  const calendarEvent: UniversalCalendarEvent = {
    id: `rdv-kit-${kit.vehicleSummary.licensePlate.replace(/[^a-zA-Z0-9]/g, "")}-${Date.now()}`,
    title: `🔧 RDV Entretien : ${kit.vehicleSummary.makeModel} [${kit.vehicleSummary.licensePlate}]`,
    startDate: new Date(Date.now() + 7 * 24 * 3600 * 1000).toISOString().slice(0, 10),
    vehicleMakeModel: kit.vehicleSummary.makeModel,
    licensePlate: kit.vehicleSummary.licensePlate,
    dueMileage: kit.vehicleSummary.currentMileage,
    estimatedCostEur: totalBudget,
    garageName: currentGarageName,
    garagePhone: currentGaragePhone,
    garageAddress: currentGarageAddress,
    description: kit.interventionsToRequest
      .map((i) => `• ${i.title}${i.specificationDetails ? ` (${i.specificationDetails})` : ""} : ~${i.estimatedBudgetEur}€`)
      .join("\n") + (kit.phoneScript ? `\n\nScript garage :\n${kit.phoneScript}` : ""),
  };
        ...
        <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 rounded-2xl p-5 text-white shadow-lg shadow-blue-950/20 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-white/10">
            ...
            <UniversalCalendarDropdown
              event={calendarEvent}
              variant="dark"
              buttonLabel="Ajouter à l'agenda (.ics, Google, Yahoo...)"
              size="md"
              filename={`rdv-entretien-${kit.vehicleSummary.licensePlate}`}
            />
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t">
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-medium transition text-xs flex-1 sm:flex-initial"
            >
          ...
          <button
            onClick={onClose}
            className="w-full sm:w-auto px-6 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold transition text-xs"
          >
            Fermer
          </button>
        </div>
